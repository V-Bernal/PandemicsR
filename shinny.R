...
dashboard_ui <- tagList(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput("gamma", "Voter: light-state gamma", min = 0, step = 0.01, max = 20, value = 0.6),
      param_help("Baseline opinion-change rate for the light shades."),
      sliderInput("gamma_dark", "Voter: dark-state gamma", min = 0, step = 0.01, max = 5, value = 0.05),
      ...
      sliderInput("beta_plus", "Schelling: beta_plus", min = 0, step = 0.005, max = 1, value = 0.04),
      ...
      sliderInput("beta_red_epi", "Epidemic: infection rate red camp", min = 0, step = 0.01, max = 2, value = 0.5),
      param_help("Dark red and light red share this infection rate in the random-mixing SIR process."),
      sliderInput("beta_blue_epi", "Epidemic: infection rate blue camp", min = 0, step = 0.01, max = 2, value = 0.16),
      param_help("Dark blue and light blue share this infection rate."),
      sliderInput("gamma_sir", "Epidemic: recovery rate", min = 0, step = 0.01, max = 2, value = 0.14),
      param_help("Common recovery rate for infected individuals."),
      sliderInput("initial_infected_fraction", "Epidemic: initial infected fraction", min = 0, step = 0.01, max = 0.5, value = 0.04),
    ...
    result <- tryCatch(
      withProgress(message = "Running simulation", value = 0.15, {
        ...
        simulate_hybrid_model(
          ...
          record_every = max(5L, as.integer(round(input$n / 12))),
