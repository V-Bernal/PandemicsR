...
visual_epidemic_camps <- function(camp_attack_rate) {
  ...
  plot_values <- ifelse(is.na(camp_attack_rate), 0, camp_attack_rate)
  text_labels <- ifelse(is.na(camp_attack_rate), "NA", sprintf("%.2f", camp_attack_rate))
  ...
  text(bp, plot_values, labels = text_labels, pos = 3, cex = 0.9)
}
